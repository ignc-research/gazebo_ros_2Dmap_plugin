...
namespace gazebo
{
                                   ...
                                   sdf::ElementPtr _sdf)
  {
    ...
    if (_sdf->HasElement("check_below"))
      check_below = _sdf->GetElement("check_below")->Get<bool>();

    step = 0.1;

    if (_sdf->HasElement("step"))
      step = _sdf->GetElement("step")->Get<double>();

    lower_bound = 0.3;

    if (_sdf->HasElement("lower_bound"))
      lower_bound = _sdf->GetElement("lower_bound")->Get<double>();
        ...
        for (int j = -1; j < 2; j++)
        {
          ...
          if (cell2index(cell_x + i, cell_y + j, cells_size_x, cells_size_y, child_index))
          {
            ...
            if (child_val != 100 && child_val != 0 && child_val != 50)
            {
              ...
              if (check_below)
              {
                for (double z = map_height_; z >= lower_bound; z -= step)
                {
                  if (worldCellIntersection(vector3d(world_x, world_y, z),
                                            map_resolution_, ray) == true)
                  {
                    cell_occupied = true;
                    break;
                  }
              ...
              {
                cell_occupied = worldCellIntersection(vector3d(world_x, world_y, map_height_),
                                                      map_resolution_, ray);
              }
